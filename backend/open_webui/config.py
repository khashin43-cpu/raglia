...
async def seed_registered_defaults():
    ...
    # Existing installations may still have Open WebUI's original English
    # examples persisted in the database. Replace only that exact legacy set,
    # leaving administrator-defined suggestions untouched.
    current_prompt_suggestions = await Config.get('ui.prompt_suggestions')
    if current_prompt_suggestions == LEGACY_DEFAULT_PROMPT_SUGGESTIONS:
        await Config.upsert({'ui.prompt_suggestions': DEFAULT_PROMPT_SUGGESTIONS})
        log.info('Migrated legacy prompt suggestions to LIA corporate defaults')
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (ЛИА + Ollama)
...
RAGFLOW_MINIO_URL = os.getenv('RAGFLOW_MINIO_URL', '')

LIA_ARIA_API_BASE_URL = os.getenv('LIA_ARIA_API_BASE_URL', '')
LIA_ARIA_ENABLED = (
    os.getenv('LIA_ARIA_ENABLED', 'true' if LIA_ARIA_API_BASE_URL else 'false').lower() == 'true'
)
LIA_ARIA_API_KEY = os.getenv('LIA_ARIA_API_KEY', '')
LIA_ARIA_MODEL = os.getenv('LIA_ARIA_MODEL', '/model')

LIA_OFFICE_ENABLED = os.getenv('LIA_OFFICE_ENABLED', 'false').lower() == 'true'
LIA_OFFICECLI_BINARY = os.getenv(
    'LIA_OFFICECLI_BINARY',
    str(Path(OPEN_WEBUI_DIR).parent / 'bin' / ('officecli.exe' if os.name == 'nt' else 'officecli')),
)
LIA_OFFICECLI_WORK_DIR = os.getenv('LIA_OFFICECLI_WORK_DIR', str(DATA_DIR / 'officecli'))
LIA_OFFICECLI_TIMEOUT = int(os.getenv('LIA_OFFICECLI_TIMEOUT', '120'))

LIA_LND_API_BASE_URL = os.getenv('LIA_LND_API_BASE_URL', '')
LIA_LND_ENABLED = (
    os.getenv('LIA_LND_ENABLED', 'true' if LIA_LND_API_BASE_URL else 'false').lower() == 'true'
)
LIA_LND_API_KEY = os.getenv('LIA_LND_API_KEY', '')
...
LEGACY_DEFAULT_PROMPT_SUGGESTIONS = [
    {
...
CORPORATE_DEFAULT_PROMPT_SUGGESTIONS = [
    {
        'title': ['Подготовить деловое письмо', 'коллегам или партнёрам'],
        'content': 'Помоги подготовить краткое деловое письмо коллегам или партнёрам.',
    },
    {
        'title': ['Составить повестку', 'для рабочей встречи'],
        'content': 'Составь повестку рабочей встречи: цели, вопросы для обсуждения и ожидаемые результаты.',
    },
    {
        'title': ['Подвести итоги', 'встречи или совещания'],
        'content': 'Помоги оформить итоги встречи: принятые решения, ответственные и следующие шаги.',
    },
    {
        'title': ['Спланировать задачу', 'этапы, сроки и риски'],
        'content': 'Помоги структурировать рабочую задачу: цель, этапы, сроки, риски и ответственные.',
    },
    {
        'title': ['Подготовить отчёт', 'кратко и по существу'],
        'content': 'Предложи структуру краткого рабочего отчёта с результатами, выводами и дальнейшими действиями.',
    },
    {
        'title': ['Улучшить формулировку', 'в корпоративном стиле'],
        'content': 'Помоги переписать текст ясно, корректно и в нейтральном корпоративном стиле.',
    },
]
...
if default_prompt_suggestions == []:
    default_prompt_suggestions = CORPORATE_DEFAULT_PROMPT_SUGGESTIONS
