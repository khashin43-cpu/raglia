...
async def chat_completion(
    ...
    # Knowledge agents are user-scoped while the native model cache is shared.
    # Refresh it for the authenticated user before resolving an agent model.
    if isinstance(model_id, str) and model_id.startswith('agent:ragflow:'):
        await get_all_models(request, user=user)
    ...
    try:
        ...
        ragflow_document_refs = form_data.pop('ragflow_document_refs', None)
        ...
        metadata = {
            ...
            'ragflow_document_refs': ragflow_document_refs if isinstance(ragflow_document_refs, list) else [],
            'model': model,
...
async def generate_messages(
    ...
    All models configured in ЛИА are accessible via this endpoint.
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for ЛИА.
