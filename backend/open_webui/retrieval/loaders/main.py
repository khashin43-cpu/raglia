...
class LegacyWordLoader:
    """Convert a legacy DOC locally and extract it through the DOCX loader."""

    def __init__(self, file_path):
        self.file_path = file_path

    def load(self) -> list[Document]:
        with tempfile.TemporaryDirectory(prefix='lia-doc-') as temp_dir:
            converted_path = Path(temp_dir) / f'{Path(self.file_path).stem}.docx'
            convert_legacy_word_to_docx(self.file_path, converted_path)
            documents = Docx2txtLoader(str(converted_path)).load()
        ...
        if (
            ...
            if file_ext == 'pdf':
                ...
                try:
                    ...
                    log.info('Converting legacy DOC locally before text extraction')
                    loader = LegacyWordLoader(file_path)
