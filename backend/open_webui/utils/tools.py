...
from open_webui.tools.officecli import (
    office_attach,
    office_deliver,
    office_preview,
    office_run,
    ragflow_save_office_document,
)
...
async def get_builtin_tools(
    ...
    # OfficeCLI is available to the dedicated document assistant and to
    # RAGFlow knowledge assistants. RAGFlow agents additionally receive a
    # guarded tool for saving the result as a new indexed document.
    agent = model.get('agent') or {}
    if agent.get('provider') in {'office', 'ragflow'} and is_builtin_tool_enabled('office'):
        builtin_functions.extend([office_run, office_preview, office_attach, office_deliver])
        if agent.get('provider') == 'ragflow':
            builtin_functions.append(ragflow_save_office_document)
