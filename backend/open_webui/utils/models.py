...
from open_webui.internal.agents import get_system_agent_models
...
async def get_all_models(request, refresh: bool = False, user: UserModel = None):
    ...
    # Deep copy the base models to avoid modifying the original list, then add
    # system agents so they participate in the native ЛИА chat pipeline.
    models = [model.copy() for model in base_models] + await get_system_agent_models(
...
async def check_model_access(user, model, db=None):
    ...
    if model.get('owned_by') == 'agent':
        if agent.get('provider') == 'ragflow':
            if agent.get('owner_id') != user.id:
...
async def get_filtered_models(models, user, db=None):
    ...
    if (
        ...
        for model in models:
            agent = model.get('agent') or {}
            if model.get('owned_by') == 'agent':
                if agent.get('system') or (
                    agent.get('provider') == 'ragflow' and agent.get('owner_id') == user.id
                ):
