<script lang="ts">
	...
	export let onSaveChatToKnowledgeBase: Function = () => {};
	export let canSaveChatToKnowledgeBase = false;
	...
	// Возможности остаются реализованными, но временно скрыты из меню «+».
	const showCaptureAction = false;
	const showAttachWebpageAction = false;
	const showAttachKnowledgeAction = false;
	const showAttachNotesAction = false;
		...
		>
			{#if tab === ''}
				<div in:fly={{ x: -20, duration: 150 }}>
					...
					{#if showAttachNotesAction && ($config?.features?.enable_notes ?? false)}
						<Tooltip
							content={fileUploadCapableModels.length !== selectedModels.length
								? $i18n.t('Model(s) do not support file upload')
								: !fileUploadEnabled
					...
					<Tooltip
						content={canSaveChatToKnowledgeBase ? '' : $i18n.t('No conversation to save')}
						className="w-full"
					>
						<button
							class="flex w-full gap-2 items-center px-3 py-1.5 text-sm select-none rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 {canSaveChatToKnowledgeBase
								? 'cursor-pointer'
								: 'cursor-not-allowed opacity-50'}"
							type="button"
							disabled={!canSaveChatToKnowledgeBase}
							on:click={() => {
								if (canSaveChatToKnowledgeBase) {
									onSaveChatToKnowledgeBase();
									show = false;
								}
							}}
						>
							<ChatBubbleOval />
							<div class="line-clamp-1">
								{$i18n.t('Save chat history to knowledge base')}
							</div>
						</button>
					</Tooltip>
