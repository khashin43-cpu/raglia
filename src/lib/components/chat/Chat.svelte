<script lang="ts">
	...
	let files = [];
	let params = {};
	let pendingRagflowDocumentRef: {
		dataset_id: string;
		document_id: string;
		name: string;
	} | null = null;

	const handleRagflowDocumentEdit = async (event: Event) => {
		const detail = (event as CustomEvent)?.detail ?? {};
		const name = String(detail.name ?? 'документ');
		const location = [detail.spaceName, detail.datasetName].filter(Boolean).join(' / ');
		const text = `Хочу изменить документ «${name}»${location ? ` из базы «${location}»` : ''}. `;
		pendingRagflowDocumentRef =
			detail.datasetId && detail.documentId
				? {
						dataset_id: String(detail.datasetId),
						document_id: String(detail.documentId),
						name
					}
				: null;
		prompt = text;
		await tick();
		await messageInput?.setText(text);
	...
	onMount(() => {
		...
		window.addEventListener('lia:edit-ragflow-document', handleRagflowDocumentEdit);
		...
		return () => {
			try {
				...
				window.removeEventListener('lia:edit-ragflow-document', handleRagflowDocumentEdit);
	...
	const sendMessageSocket = async (
		...
		const res = await generateOpenAIChatCompletion(
			localStorage.token,
			{
				...
				ragflow_document_refs: pendingRagflowDocumentRef ? [pendingRagflowDocumentRef] : undefined,
		...
		pendingRagflowDocumentRef = null;
