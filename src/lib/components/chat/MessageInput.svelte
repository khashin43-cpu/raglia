<script lang="ts">
	...
	import AgentSelector from './MessageInput/AgentSelector.svelte';
	...
	import SaveChatToKnowledgeModal from './MessageInput/SaveChatToKnowledgeModal.svelte';
	...
	let showSaveChatToKnowledgeModal = false;
	let inputVariablesModalCallback = (variableValues) => {};
	let inputVariables = {};
	let inputVariableValues = {};
	let currentChatMessages = [];
	$: currentChatMessages = history?.currentId ? createMessagesList(history, history.currentId) : [];
	...
	let loaded = false;
	let recording = false;
	// Голосовой режим отключён в ЛИА. Голосовой ввод через микрофон остаётся доступен.
	const showVoiceModeAction = false;
...
<SaveChatToKnowledgeModal
	bind:show={showSaveChatToKnowledgeModal}
	title={$chatTitle}
							...
							<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
								<div class="ml-1 self-end flex items-center flex-1 min-w-0">
									<InputMenu
										...
										onSaveChatToKnowledgeBase={() => {
											showSaveChatToKnowledgeModal = true;
										}}
										canSaveChatToKnowledgeBase={currentChatMessages.length > 0}
										onClose={async () => {
											await tick();
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]">
									...
									{:else}
										...
										{#if showVoiceModeAction && prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												<!-- {$i18n.t('Call')} -->
