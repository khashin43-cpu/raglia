<script lang="ts">
	...
	import OfficeDocumentCard from './OfficeDocumentCard.svelte';
	...
	const isOfficePresentationEmbed = (embed: unknown) =>
		typeof embed === 'string' &&
		embed.includes('OfficeCli HTML Preview Stylesheet') &&
		/<title>[^<]*\.pptx<\/title>/i.test(embed);

	const getPresentationFile = () =>
		message?.files?.find((file) => (file?.name?.split('.').pop() ?? '').toLowerCase() === 'pptx') ??
		null;

	const downloadPresentation = (file) => {
		if (!file?.url) return;
							...
							>
								{#each message.files.filter((f) => ['image', 'file'].includes(f.type)) as file}
									<div>
										...
										{:else if file.office || ['docx', 'xlsx', 'pptx'].includes((file.name
														?.split('.')
														.pop() ?? '').toLowerCase())}
											<OfficeDocumentCard
												{file}
												showPreviewHint={(message?.embeds?.length ?? 0) > 0}
											/>
										{:else}
											<FileItem
												item={file}
						...
						{#if message?.embeds && message.embeds.length > 0}
							...
							>
								{#each message.embeds as embed, idx}
									{@const isPresentation = isOfficePresentationEmbed(embed)}
									{@const presentationFile = isPresentation ? getPresentationFile() : null}
									<div
										class="my-2 w-full {isPresentation
											? 'overflow-hidden rounded-2xl border border-gray-200 bg-gray-50 shadow-sm dark:border-gray-700 dark:bg-gray-900'
											: ''}"
										id={`${message.id}-embeds-${idx}`}
									>
										{#if isPresentation}
											<div
												class="flex min-h-12 items-center justify-between gap-3 border-b border-gray-200 bg-white px-4 py-2.5 dark:border-gray-700 dark:bg-gray-900"
											>
												<div class="min-w-0">
													<div class="text-sm font-semibold text-gray-900 dark:text-gray-100">
														Предпросмотр презентации
													</div>
													{#if presentationFile?.name}
												...
												{#if presentationFile}
													<button
														type="button"
														class="shrink-0 rounded-xl bg-orange-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-orange-700 flex items-center gap-1.5"
														on:click={() => downloadPresentation(presentationFile)}
														aria-label={`Скачать ${presentationFile.name ?? 'презентацию'}`}
													>
														<svg
															viewBox="0 0 24 24"
															fill="none"
										...
										<FullHeightIframe
											src={embed}
											title={isPresentation ? 'Предпросмотр презентации' : 'Встроенный результат'}
											iframeClassName={isPresentation
												? 'block w-full h-[clamp(32rem,75vh,52rem)] bg-gray-950'
												: 'w-full rounded-2xl'}
											allowScripts={true}
											allowForms={true}
