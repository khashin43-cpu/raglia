...
Our goal is to protect ЛИА's users and their data, and to handle security reports with a clear, consistent, and publicly documented process.
...
The **CVE Program rules** (and CNA operational rules) are the **baseline** for all CVE handling here, and this policy operates within them. Under those rules, the determination of whether a report constitutes a security vulnerability in ЛИА is the vendor's to make; this policy documents the criteria by which we exercise that determination. Where the rules are silent, they still apply; where this policy specifies how we apply them to ЛИА, it does so as the vendor's published disposition criteria, not as a replacement for or exception to the program rules.
...
3. **In-Depth Understanding**: Reports must reflect a clear understanding of the codebase, how ЛИА is used and provide specific details about the vulnerability, including the affected components and potential impacts.
...
6. **Default Configuration Testing**: Vulnerability reports must be tested and reproducible using ЛИА's out-of-the-box default configuration. Claims of vulnerabilities that only manifest with explicitly weakened security settings may be discarded, unless they are covered by the following exception:
...
7. **Threat Model Understanding Required**: Reports must demonstrate understanding of ЛИА's self-hosted, single-tenant, authenticated, extensible, role-based access control architecture. Comparing ЛИА to services with fundamentally different security models without acknowledging the architectural differences may result in report rejection.
...
We aim to triage new reports, ship fixes, and publish advisories promptly. However, due to the very high volume of incoming vulnerability reports, issues, discussions, pull requests, and general project maintenance — lately compounded by a high number of (AI-generated) reports — our capacity to respond is limited. ЛИА is a community-driven project maintained by a small team, and security reports are handled alongside all other project responsibilities.
...
This confidential, responsible disclosure process exists to give us time to fix bugs, publish fixes and alert users once a fix is ready. The entire premise of responsible disclosure is to **protect users from vulnerabilities**. Therefore, premature disclosure undermines the security of all ЛИА users and **violates the trust** inherent in the responsible disclosure process. **Reporters who prematurely publicly disclose vulnerability details before official publication <ins>WILL BE PERMANENTLY BANNED from future reporting.</ins>**
...
- **Configuration help:** Ask the community for help and guidance on our [Discord Server](https://discord.gg/5rJgQTnV4s) or on [Reddit](https://www.reddit.com/r/ЛИА/)
